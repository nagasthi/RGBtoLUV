...
# print(LuvMatrix)
# equ = cv2.equalizeHist(LuvMatrix[:,:,0]);
# print("############")
# print(equ)
...
#calculating cumulative frequency sum 
f=np.cumsum(h)

#calculating ( f(i) + f(i-1) ) /2 
for i in range(0, 101):
    if (i==0):
        I[i] = math.floor(f[i] * 101 / (2 * f[100]))
    else:
        I[i] = math.floor((f[i - 1] + f[i]) * 101 / (2 * f[100]))
